    ...
    */
     ...
     //    ]);
        ...
        foreach ($datas as $value) {
            foreach ($value as $data) {
                ...
                if($status){
                    if($data['school_admission_no'] !=''){
                        $admissionNo = $data['school_admission_no'];
                        $status = !empty($admissionNo) ? true : false;
                    }
                }else{
                    $status = false;
                }
                if($status){
                    if($data['class_roll_no'] !=''){
                        $classRollNo = $data['class_roll_no'];
                        $status = !empty($classRollNo) ? true : false;
                    }
                }else{
                    $status = false;
                } 
                if($status){
                    if($data['student_status'] !=''){
                        $student_status = ($data['student_status'] == 'Running' ? 'R': '' ($data['student_status'] == 'Pass'? 'P': '' ($data['student_status'] == 'Fail' ? 'F': '')));
                        $status = !empty($student_status) ? true : false;
                    // dd($student_status);
                    }
                }else{
                    $status = false;
                ...
                if($status){
                    if($data['spec_ailment'] !=''){
                        $spec_ailment = $data['spec_ailment'];
                        $status = !empty($spec_ailment) ? true : false;
                    }
                }else{
                    $status = false;
                } 
    ...
    //end send user name and password using email and SMS.............
                    ...
                    $errors[] = [
                        ...
                        'school_admission_no' => $data['school_admission_no'],
